...
void PaScaL_TDMA_plan_many_create(ptdma_plan_many* plan, int n_sys,
    ...
    for (int i = 0; i < nprocs; ++i) {
        bigsize[0] = ns_rd; bigsize[1] = nr_rd;
        subsize[0] = ns_rt_array[i]; subsize[1] = nr_rd;
        ...
        bigsize[0] = ns_rt; bigsize[1] = nr_rt;
        subsize[0] = ns_rt; subsize[1] = nr_rd;
        start[0] = 0; start[1] = nr_rd * i;
        MPI_Type_create_subarray(2, bigsize, subsize, start,
                                 MPI_ORDER_C, MPI_DOUBLE,
                                 &plan->ddtype_BS[i]);
...
void PaScaL_TDMA_many_solve(ptdma_plan_many* plan, double* A, double* B,
    ...
    MPI_Waitall(4, request, MPI_STATUSES_IGNORE);
    timer::timer_stamp(11, stamp_tdma);
